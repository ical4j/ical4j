...
- **Refactor F5 (Co-locate iTIP rules in `validate/`)**: Move the `methodValidators` maps out of `model/component/VEvent.java`, `VToDo.java`, `VJournal.java`, `VFreeBusy.java` into a single `validate/ITIPRuleRegistry` that holds the per-(component, method) rule sets. The model classes' `validate(Method)` methods become thin dispatchers that consult the registry. **Deferred to a follow-up change** (originally planned to be in this change as well): deletion of the six `@Deprecated validate/component/*Validator.java` subclasses. During implementation we discovered these classes carry ~270 lines of non-trivial extra validation logic (alarm handling, STATUS allowed-value checks per VTODO state, STANDARD/DAYLIGHT observance presence) — they're load-bearing, not just rule-set wrappers. Consolidating their behaviour into the registry warrants its own design rather than being squeezed into this change.
...
- **Out of scope (deferred to follow-up changes)**: Cross-property constraint vocabulary (e.g., RECURRENCE-ID type ≡ DTSTART type), systematic row-by-row audit of every RFC 5546 §3.x/§4.x table, property value-set enum constraints (CLASS, TRANSP, STATUS per-component, PARTSTAT, ROLE, CUTYPE), deletion of the `@Deprecated validate/component/*Validator.java` subclasses (their alarm-handling / STATUS-token / observance-presence logic needs to be ported to the registry or absorbed elsewhere first). These are larger pieces of work and warrant their own change proposals.
