...
- [x] 4.2 Subtract files with `suite()` (they belong to Phase 4) and base classes
...
- [x] 5.3 **Sub-phase 4c (atomic commit)**: `model/ComponentTest` + `model/CalendarComponentTest` + all ~12 `model/component/*` subclasses
   ...
   - [x] 5.3.3 Migrate each `model/component/*` subclass
   - [x] 5.3.4 Run `./gradlew test` — all green
   - [x] 5.3.5 Commit Sub-phase 4c
...
- [x] 7.2 No new compiler warnings introduced
- [x] 7.3 JaCoCo coverage report shows no regression > 1% from previous phase
- [x] 7.4 Test invocation count is monotonic: each phase's total ≥ previous phase's total (no silent test loss)
- [x] 7.5 **Spock/Groovy untouched check**: `git diff --name-only <phase-base>..HEAD -- src/test/groovy/` returns zero files; any non-empty diff is a scope violation and must be reverted
- [x] 7.6 **Spock test count unchanged**: count of Spock specifications reported by Gradle is the same as the pre-migration baseline (run once at start, compare after each phase)
