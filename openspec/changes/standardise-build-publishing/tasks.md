...
- [ ] 1.3 Verify 1.2 by publishing a snapshot from `ical4j-extensions` CI. **Unblocked 2026-08-19: CI secrets now set org-wide. Superseded in practice by the full migration (3.3) — verification is the snapshot publish when PR ical4j/ical4j-extensions#41 merges to `develop`.**
...
- [ ] 3.1 `ical4j-zoneinfo-outlook` (pilot — smallest build, single dependency, no revapi) **Migrated 2026-08-19; PR ical4j/ical4j-zoneinfo-outlook#35. Only CI snapshot (3.1.7) outstanding.**
  ...
  - [ ] 3.1.7 Publish snapshot from CI; confirm it lands on the Central Portal **Unblocked 2026-08-19: `CENTRAL_PORTAL_USERNAME/PASSWORD` + `GPG_SIGNING_KEY/GPG_SIGNING_PASSWORD` set as org-level secrets (visibility: all repos). Runs when the PR merges to `develop`.**
